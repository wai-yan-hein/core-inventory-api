...
alter table order_his
add column inv_update bit(1) not null default 0 after post;

alter table transfer_his
add column skip_inv bit(1) default false after print_count;

create table pur_io_join (
  pur_vou_no varchar(25) not null,
  io_vou_no varchar(25) not null,
  comp_code varchar(15) not null,
  primary key (pur_vou_no,io_vou_no,comp_code)
) engine=innodb default charset=utf8mb3 collate=utf8mb3_general_ci;

alter table stock_in_out
add column post bit(1);

create table stock_unit_price (
  stock_code varchar(15) not null,
  comp_code varchar(15) not null,
  unique_id int(11) not null,
  unit varchar(15) not null,
  sale_price_n double default null,
  sale_price_a double default null,
  sale_price_b double default null,
  sale_price_c double default null,
  sale_price_d double default null,
  sale_price_e double default null,
  updated_date timestamp not null default current_timestamp() on update current_timestamp(),
  rel_code varchar(15) null,
  primary key (stock_code,comp_code,unit)
...
alter table payment_his
add column dept_code varchar(15);

create table f_stock (
  f_code varchar(15) not null,
  mac_id int(11) not null,
  primary key (f_code,mac_id)
) engine=innodb default charset=utf8mb3 collate=utf8mb3_general_ci;

drop table if exists reorder_level;
create table reorder_level (
  comp_code varchar(15) not null,
  loc_code varchar(15) not null,
  stock_code varchar(15) not null,
  min_qty double default 0,
  min_unit varchar(15) default null,
  max_qty double default 0,
  max_unit varchar(15) default null,
  primary key (comp_code,loc_code,stock_code)
) engine=innodb default charset=utf8 collate=utf8_general_ci;

alter table pur_his_detail
add column foc bit(1) not null default 0 after weight_unit,
add column dis_amt double(20,3) null after foc;
